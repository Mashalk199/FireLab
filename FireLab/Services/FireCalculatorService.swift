...
/**
 ...
 This version steps the simulation **month-by-month** instead of day-by-day to improve performance, while keeping
 the overall logic as close as possible to the original daily version.
 */
struct FireCalculatorService {
    private let dataService: FinancialDataFetching
    /// Single forecaster – either available or not. We assume it provides *monthly* returns.
    private let fullForecaster: Forecasting?

    init(
        dataService: FinancialDataFetching = FinancialDataService(),
        fullForecaster: Forecasting? = try? MLForecastService(modelType: .monthly)
    ) {
        self.dataService = dataService
        self.fullForecaster = fullForecaster
    }

    /// Used only for age calculations (date differences in years)
    private let DAYS_IN_YEAR: Double = 365.0
    /// Number of months in a year for monthly compounding
    private let MONTHS_IN_YEAR: Double = 12.0

    // Nominal *monthly* factor for DEBT interest compounding
    private func monthlyDebtFactor(_ annualNominal: Double) -> Double {
        pow(1.0 + max(0.0, annualNominal), 1.0 / MONTHS_IN_YEAR)
    }

    /**
     Converts annual return to the amount compounded monthly (real return after inflation).
     ...
     - Here we step **monthly** rather than daily.
     */
    private func getMonthlyReturn(_ percentage: Double, _ annual_inflation: Double) -> Double {
        pow(((1.0 + percentage) / (1.0 + annual_inflation)), (1.0 / MONTHS_IN_YEAR))
    }

    // for test
    func _test_monthlyReturn(annualReturn: Double, inflation: Double) -> Double {
        pow(((1.0 + annualReturn) / (1.0 + inflation)), (1.0 / MONTHS_IN_YEAR))
    }
    ...
    func calculateRetirement(inputs: FireInputs) async throws -> Result {
        ...
        let monthlyTotal  = yearlyTotalFI / MONTHS_IN_YEAR                   // monthly FI contribution
        ...
        // -----------------
        // Phase A: debts (now month-by-month)
        // -----------------
        ...
        /*
         ...
         We now step this phase **month by month** instead of day by day.
         */

        // Create a debts tuple to avoid using loanItems observableObject inefficiencies
        var debts: [(name: String, balance: Double, annualRate: Double, minMonthly: Double, monthlyFactor: Double)] =
        ...
        // Limit Phase A to the time until age 67 (in months)
        let maxDebtMonths = Int(max(0.0, (67.0 - ageYearsNow)) * MONTHS_IN_YEAR)
        ...
        if !debts.isEmpty {
            ...
            while debtMonths < maxDebtMonths && !allCleared() {
                ...
                if debtMonths % 12 == 0 {
                    ...
                    print("Month \(debtMonths): debts=[\(hb)]")
                }
            }
        }
        ...
        print("Phase A finished after \(debtMonths) months. Debts now: [\(endDebtsStr)]")

        // Approximate days from months so the Result API stays in terms of "days"
        let debtClearDaysApprox = debtMonths * 30

        // If we hit 67 (or otherwise didn’t clear all debts), stop here.
        if !remainingAfterPhaseA.isEmpty {
            let retirementDate = Calendar.current.date(byAdding: .day, value: debtClearDaysApprox, to: today) ?? today
            return Result(
                workingMonths: debtMonths,
        ...
        var etfIndices: [Int] = inputs.investmentItems.indices.filter {
            inputs.investmentItems[$0].type == .etf &&
        ...
        if let fullForecaster {
            ...
            // We only need about 2 years of monthly forecasts → 24 months
            let forecastMonths = 24

            for i in etfIndices {
                if let prices = etfData[i] {
                    predReturns[i] = try fullForecaster.predictReturns(
                        closes: prices,
                        steps: forecastMonths
                    )
                }
            }
        } else {
            // No forecaster available – disable ML usage.
            etfIndices.removeAll()
        }

        // -----------------
        // Phase C: invest and find retirement (bisection approach, month-by-month)
        // -----------------

        /*
         In Phase C we go through the users financial life from now until they retire, and calculate whether retirement is possible for every month.
         If not, the user works an additional month, otherwise they stop working. Funds are split between brokerage and super investments, so a binary-search bisection
        ...
        let workingMonthsOffset   = debtMonths
        let months_to_60          = max(0, Int((60.0 - ageYearsNow) * MONTHS_IN_YEAR) - workingMonthsOffset)
        let months_to_67          = max(0, Int((67.0 - ageYearsNow) * MONTHS_IN_YEAR) - workingMonthsOffset)
        ...
        for _ in 1...8 {
            ...
            while (workingMonths < months_to_67) && !(retiredBroker && retiredSuper) {
                ...
                for i in debtsTemplate.indices where debtsTemplate[i].balance > eps {
                    debtsTemplate[i].balance *= debtsTemplate[i].monthlyFactor
                }

                // Tracks the current amount of the monthly FI contribution "currently" left to put towards loans and investments.
                var fi_left = monthlyFI
                ...
                for i in debtsTemplate.indices {
                    ...
                    let dueMin = min(debtsTemplate[i].minMonthly, debtsTemplate[i].balance)
                    let pay    = min(fi_left, dueMin)
                    debtsTemplate[i].balance -= pay
                    fi_left -= pay
                }

                // brokerCont stores the proportion of money of the remainder of the FI contribution to be put towards all brokerage investments. Same with superCont for superannuation
                ...
                for j in brokerListGrowth.indices {
                    ...
                    // We only use machine learning predictions for the first 2 years (24 months),
                    // after that we rely on the user's provided return
                    if workingMonths < 24 && etfIndices.contains(j) {
                        if let returns = predReturns[j], workingMonths < returns.count {
                            brokerListGrowth[j] *= (1 + returns[workingMonths] / 100.0)
                        }
                    } else {
                ...
                // Grow super by 1 month of investments
                ...
                if workingMonths % 12 == 0 {
                    ...
                    print("Epoch \(epochIndex) – month \(workingMonths): debts=[\(hb)]")
                }
                ...
                /* Here, at the current workingMonths, we reduce the current value of the brokerage investment to basically
                 ...
                 will continue to grow via more working months. */

                while sumAll() >= monthlyExpenses &&
                      !retiredBroker &&
                    ...
                    for j in portfolioList.indices {
                        ...
                        let t = workingMonths + retiredMonths_tmp
                        if t < 24 && etfIndices.contains(j) {
                            if let returns = predReturns[j], t < returns.count {
                                portfolioList[j] *= (1 + returns[t] / 100.0)
                            }
                        } else {
                    ...
                    // Calculate the total debt minimums due this month
                    var monthlyDebtDue = 0.0
                    for i in tempDebts.indices where tempDebts[i].balance > eps {
                        tempDebts[i].balance *= tempDebts[i].monthlyFactor // optional inner-month accrual
                        monthlyDebtDue += min(tempDebts[i].minMonthly, tempDebts[i].balance)
                    }
                    ...
                    if monthlyDebtDue > 0 {
                        ...
                        for i in tempDebts.indices where toReduce > 0 && tempDebts[i].balance > eps {
                            ...
                            let due = min(tempDebts[i].minMonthly, tempDebts[i].balance)
                    ...
                    retiredMonths_tmp += 1
                    if workingMonths + retiredMonths_tmp >= months_to_60 {
                        retiredBroker = true
                    }
                }

                // super post-60 feasibility (compounds to 60 then simulate future months)

                // how many months until 60 from "now"
                let remain_to_60 = max(0, months_to_60 - workingMonths)
                ...
                while temp_super >= monthlyExpenses && !retiredSuper {
                    for i in tempDebtsSuper.indices where tempDebtsSuper[i].balance > eps {
                        tempDebtsSuper[i].balance *= tempDebtsSuper[i].monthlyFactor
                    }
                    ...
                    var monthlyDebtDue = 0.0
                    for d in tempDebtsSuper where d.balance > eps { monthlyDebtDue += min(d.minMonthly, d.balance) }
                    let pay = min(monthlyDebtDue, temp_super)
                    temp_super -= pay
                    var toReduce = pay
                    for i in tempDebtsSuper.indices where toReduce > 0 && tempDebtsSuper[i].balance > eps {
                        let due = min(tempDebtsSuper[i].minMonthly, tempDebtsSuper[i].balance)
                        let take = min(toReduce, due)
                    ...
                    if temp_super < monthlyExpenses { break }
                    temp_super -= monthlyExpenses

                    retiredSuperMonths_tmp += 1
                    if retiredSuperMonths_tmp >= months_during_super {
                        retiredSuper = true
                    }
                }
            ...
            while (portfolioList.reduce(0,+) + tempCurr.reduce(0,+)) >= monthlyExpenses {
                ...
                for j in portfolioList.indices {
                    ...
                    let t = workingMonths + totalRetiredMonths
                    if t < 24 && etfIndices.contains(j) {
                        if let returns = predReturns[j], t < returns.count {
                            portfolioList[j] *= (1 + returns[t] / 100.0)
                        }
                ...
                totalRetiredMonths += 1
            }

            let remain_to_60 = max(0, months_to_60 - workingMonths)
            var totalRetiredSuperMonths = 0
            var temp_super = superGrowth * pow(superFactor, Double(remain_to_60))
            var tempDebtsSuper2 = debtsTemplate

            // This loop gives the true value of how long the super could fully last
            while temp_super >= monthlyExpenses {
                for i in tempDebtsSuper2.indices where tempDebtsSuper2[i].balance > eps {
                    tempDebtsSuper2[i].balance *= tempDebtsSuper2[i].monthlyFactor
                }
                // Grow super
                temp_super *= superFactor
                // Pay debt minimums
                var monthlyDebtDue = 0.0
                for d in tempDebtsSuper2 where d.balance > eps { monthlyDebtDue += min(d.minMonthly, d.balance) }
                let pay = min(monthlyDebtDue, temp_super)
                temp_super -= pay
                var toReduce = pay
                for i in tempDebtsSuper2.indices where toReduce > 0 && tempDebtsSuper2[i].balance > eps {
                    let due = min(tempDebtsSuper2[i].minMonthly, tempDebtsSuper2[i].balance)
                    let take = min(toReduce, due)
                ...
                totalRetiredSuperMonths += 1
            }

            let potential_pre60 = max(1, months_to_60 - workingMonths) // avoid /0 when retiring at/after 60
            ...
            /* This variable is the magnitude of how much brokerage growth is achieved in this epoch (eg. totalRetiredMonths) compared to the necessary
             minimum amount (eg. potential_pre60) */
            let pre60_growth_tmp  = Double(totalRetiredMonths) / Double(potential_pre60)
            let post60_growth_tmp = Double(totalRetiredSuperMonths) / Double(months_during_super)
            ...
            print("Epoch \(epochIndex) finished after \(workingMonths) months")
            ...
            /* If the ratio of "total potential" retired months before 60 compared to the minimum required retired months
            before 60 is less than the ratio of "total potential" retired months before 67 to
            minimum required retired months AFTER 60, then increase the proportion of funds sent to brokerage */
        ...
        // final summary values (convert months → approximate days for the Result)
        let totalMonthsWorked = workingMonths + workingMonthsOffset
        let workingDaysApprox = totalMonthsWorked * 30

        let retirementDate = Calendar.current.date(byAdding: .month, value: totalMonthsWorked, to: today) ?? today
        ...
        let remain_to_60_final = max(0, months_to_60 - workingMonths)
        let superAt60 = superGrowth * pow(superFactor, Double(remain_to_60_final))

        print("\n=== Final Summary (monthly sim) ===")
        print("Working months: \(workingMonths)")
        print("Approx working days: \(workingDaysApprox)")
        ...
        return Result(
            workingMonths: totalMonthsWorked,
            retirementDate: retirementDate,
